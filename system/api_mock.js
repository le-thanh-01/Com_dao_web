 ...
 *
 * Dữ liệu sẽ được lưu trữ tạm thời dưới dạng đối tượng Mock_Database.DB trước khi
 * có database thật. Mock_Database.DB được lưu trên client trong local store
 * với username là Mock_Database.simulateDB.
 *
...
export const Mock_Database = {
  /* ── Dữ liệu gốc (thay bằng Mock_Database.simulateDB khi chuyển qua lưu trữ
  ...
  DB: {
    ...
    notices: [
      ...
      },
      {
        ...
        date: "2024-02-28",
        summary:
          "Đơn hàng đang được chuẩn bị. Dự kiến phục vụ trong 15-20 phút.",
        content:
  ...
  /* ── Utility ── */
  delay: (min = 400, max = 900) =>
    new Promise((res) => setTimeout(res, min + Math.random() * (max - min))),

  respond: (data) => ({ data, error: null }),
  fail: (msg) => ({ data: null, error: msg }),

  deepClone: (val) => JSON.parse(JSON.stringify(val)),
};

const localDB = getSafeData("simulateDB");
let targetDB = localDB ? localDB : Mock_Database.DB;
/**
 * Đối tượng Mock_Database.simulateDB là một Proxy để vừa chứa dữ liệu của Mock_Database.DB
 * (hoặc Mock_Database.simulateDB trong local storage) vừa lưu dữ liệu mới
 * vào local storage khi được các component tác động.
 *
 * Tương tác với Đối tượng Mock_Database.simulateDB tương tự như tương tác với
...
Mock_Database.simulateDB = new Proxy(targetDB, DBHandler);
...
export async function fetchProducts() {
  ...
  return Mock_Database.respond(
    Mock_Database.deepClone(Mock_Database.simulateDB.products),
  );
}
...
export async function fetchCategories() {
  await Mock_Database.delay(300, 600);
  return Mock_Database.respond(
    Mock_Database.deepClone(Mock_Database.simulateDB.categories),
  );
}
...
export async function login({ username, password }) {
  await Mock_Database.delay(600, 1100);
  if (!username || !password)
    return Mock_Database.fail("Vui lòng nhập đầy đủ thông tin.");
  if (
    Mock_Database.simulateDB.user.password !== password ||
    (Mock_Database.simulateDB.user.phone !== username &&
      Mock_Database.simulateDB.user.email !== username)
  )
    return Mock_Database.fail("Mật khẩu hoặc SĐT không đúng.");
  Mock_Database.simulateDB.loginState = true;
  return Mock_Database.respond({
    token: "mock-jwt-token-abc123",
    user: Mock_Database.deepClone(Mock_Database.simulateDB.user),
  });
}

/** POST /user/register */
export async function register(fields) {
  await Mock_Database.delay(700, 1200);
  const { firstName, lastName, email, phone, password } = fields;
  if (!firstName || !lastName || (!email && !phone) || !password)
    return Mock_Database.fail("Vui lòng điền đầy đủ thông tin.");

  if (password.length < 6)
    return Mock_Database.fail("Mật khẩu phải có ít nhất 6 ký tự.");
  Mock_Database.simulateDB.user = {
    ...Mock_Database.simulateDB.user,
    ...fields,
  };
  return Mock_Database.respond({
...
export async function logout() {
  ...
  return Mock_Database.respond({ success: true });
}
...
export async function fetchLoginState() {
  ...
  return Mock_Database.respond(
    Mock_Database.deepClone(Mock_Database.simulateDB.loginState),
  );
}
...
export async function fetchProfile() {
  await Mock_Database.delay(500, 900);
  return Mock_Database.respond(
    Mock_Database.deepClone(Mock_Database.simulateDB.user),
  );
}
...
export async function fetchSettings() {
  await Mock_Database.delay(400, 800);
  return Mock_Database.respond(
    Mock_Database.deepClone(Mock_Database.simulateDB.settings),
  );
}
...
export async function updateSettings(fields) {
  await Mock_Database.delay(500, 900);
  Mock_Database.simulateDB.settings = {
    ...Mock_Database.simulateDB.settings,
    ...fields,
  };
  return Mock_Database.respond({
    success: true,
    settings: Mock_Database.deepClone(Mock_Database.simulateDB.settings),
  });
}
...
export async function placeOrder(payload) {
  ...
  return Mock_Database.respond({
    success: true,
    order: Mock_Database.deepClone(newOrder),
  });
}
...
export async function cancelOrder(id) {
  ...
  if (!order) return Mock_Database.fail("Không tìm thấy đơn hàng.");
  if (order.status !== "pending" && order.status !== "waiting")
    return Mock_Database.fail("Chỉ có thể huỷ đơn hàng đang chờ xử lý.");
  order.status = "cancelled";
  return Mock_Database.respond({
    success: true,
    order: Mock_Database.deepClone(order),
  });
}
...
export async function confirmDelivery(id) {
  ...
  if (order.status !== "pending")
    return Mock_Database.fail("Đơn hàng không ở trạng thái có thể xác nhận.");
...
export async function fetchNotices() {
  await Mock_Database.delay(400, 800);
  return Mock_Database.respond(
    Mock_Database.deepClone(Mock_Database.simulateDB.notices),
  );
}
...
export async function validatePromo(code) {
  ...
  if (!promo)
    return Mock_Database.fail("Mã giảm giá không hợp lệ hoặc đã hết hạn.");
  return Mock_Database.respond(promo);
}
